...
void WebServer::start() {
    ...
    while(!is_close_) {
        if(timeout_ms_ > 0) time_ms = timer_->get_next_tick();
        int event_cnt = epoller_->wait(time_ms);
        for(int i = 0; i< event_cnt; ++i) {
            int fd = epoller_->get_event_fd(i);
            size_t events = epoller_->get_events(i);
            if(fd == listen_fd_) deal_listen();
            else if(events & (EPOLLRDHUP | EPOLLHUP | EPOLLERR)) close_conn(&users_[fd]);
            else if(events & EPOLLIN) deal_read(&users_[fd]);
            else if(events & EPOLLOUT) deal_write(&users_[fd]);
...
void WebServer::add_client(int fd, sockaddr_in addr) {
    ...
    if(timeout_ms_ > 0) timer_->add(fd, timeout_ms_, std::bind(&WebServer::close_conn, this, &users_[fd]));
...
void WebServer::extend_time(HttpConn* client) {
    if(timeout_ms_) timer_->adjust(client->get_fd(), timeout_ms_);
}

void WebServer::on_process(HttpConn* client) {
    if(client->process()) epoller_->mod_fd(client->get_fd(), conn_event_ | EPOLLOUT);
    else epoller_->mod_fd(client->get_fd(), conn_event_ | EPOLLIN);
}
